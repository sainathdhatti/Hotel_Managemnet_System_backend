...
export class OrderService {
  ...
  async createOrder(createOrderDto: CreateOrderDto) {
    ...
    if (!booking) {
      this.logger.error(
        `Booking with ID ${createOrderDto.bookingId} not found`,
      );
      throw new NotFoundException(
        `Booking with ID ${createOrderDto.bookingId} not found`,
      );
    ...
    for (const itemDto of createOrderDto.orderItems) {
      const foodItem = await this.foodItemRepository.findOneBy({
        food_id: itemDto.foodItemId,
      });
      if (!foodItem) {
        this.logger.error(`Food item with ID ${itemDto.foodItemId} not found`);
        throw new NotFoundException(
          `Food item with ID ${itemDto.foodItemId} not found`,
        );
      }
      ...
      // Assuming this is within a method
      order.booking = booking; // From Spa branch

      this.logger.log('totalamount:', totalPrice); // From master branch
    ...
    return plainToClass(FoodOrder, savedOrder, {
  ...
  async getAllOrders() {
    const orders = await this.orderRepository.find({
      relations: ['orderItems', 'user', 'booking'],
    });
  ...
  async updateOrder(id: number, updateOrderDto: UpdateOrderDto) {
    ...
    if (updateOrderDto.status) {
      this.logger.log(
        `Updating status from ${order.status} to ${updateOrderDto.status}`,
      );
    ...
    return plainToClass(FoodOrder, updatedOrder, {
      excludeExtraneousValues: true,
    });
  }
  ...
  private formatOrderResponse(order: FoodOrder) {
    return {
      ...
      delivered_time: order.delivered_time
        ? order.delivered_time.toISOString()
        : null,
      status: order.status,
